...
public class StaticArrayDeque<T> implements Deque<T> {
    ...
    private int headIndex = 0;  // индекс для записи/извлечения элемента в начало очереди (в основной случае указывает за заполненную ячейку)

    private int tailIndex = 0;  // индекс для записи/извлечения элемента в конец очереди (в основной случае указывает за заполненную ячейку)
