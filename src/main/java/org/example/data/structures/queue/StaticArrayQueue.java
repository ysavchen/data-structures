...
public class StaticArrayQueue<T> implements Queue<T> {
    ...
    private int headIndex = 0;  // индекс для извлечения элемента из очереди (в основном случае указывает за заполненную ячейку)

    private int tailIndex = 0;  // индекс для добавления элемента в очередь (в основном случае указывает за пустую ячейку)
